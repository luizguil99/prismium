    ...
    if (selectedFile && currentDocument) {
      ...
      if (!existingHistory) {
        if (normalizedCurrentContent !== normalizedOriginalContent) {
          ...
          // Filtrar apenas mudanças significativas para a visualização na FileTree
          const significantChanges = newChanges.filter(change => 
          ...
          setFileHistory(prev => ({
            ...
            [selectedFile]: {
              ...
              changes: significantChanges.length > 0 ? significantChanges : newChanges,
              versions: [{
                timestamp: Date.now(),
      ...
      if (unifiedDiff) {
        ...
        if (hasSignificantChanges) {
          // Assegurar que as mudanças são detectadas corretamente para a FileTree
          const significantChanges = newChanges.filter(change => 
            (change.added || change.removed) && change.value.trim().length > 0
          );
