...
export const DomainSettingsModal = ({
  ...
  // Inicializar o novo domínio com o domínio atual
  useEffect(() => {
    if (currentDomain) {
      // É um domínio personalizado
      setNewDomain(cleanDomain(currentDomain));
    }
  }, [currentDomain]);
  ...
  const updateDomain = async () => {
    // Usar o domínio personalizado
    const domainToUse = newDomain.trim();
    ...
    if (!validateCustomDomain(domainToUse)) {
    ...
    try {
      // Prepare o payload para domínio personalizado
      const payload = {
